...
newman.run({
    ...
    reporter : { htmlextra : { export : './results/categories-post.html'} },
    folder: "products - post",
    environment: environments,
    iterationData: FAKER_DIR + '/' + 'products.json'
}, function (err) {
    if (err) { throw err; }
    console.log('\`\/api\/v1\/products\` run complete!');
});